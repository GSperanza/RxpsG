...
#--- IMPORT WINDOW
       ...
       RightGbox <- gvbox(expand=TRUE, fill=TRUE, container = InputMainGroup)

#--- define INPUT Window
       InputGroup <- gpanedgroup(expand=TRUE, horizontal=FALSE, container = RightGbox)
...
#--- define OUTPUT Window
       WinOUT <- gvbox(expand=TRUE, fill=TRUE, spacing=5, container = InputGroup)
...
#--- controls to set the arguments for `read.table`
       read.opt <- gframe(text=" Import Options ", horizontal = FALSE, container = LeftGbox)
