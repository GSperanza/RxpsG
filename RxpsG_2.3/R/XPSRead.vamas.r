...
# =================================================================
## Creation of XPSCoreLine and store information
   	tmpmat <- matrix(data=as.numeric(Y),   #read Y data and, if rpesent, the Transmission
   		          	    	ncol=as.numeric(corresponding_variables_num),
   				              byrow=TRUE)
   	colnames(tmpmat) <- sapply(corresponding_variables, function(x) unlist(x$label)) #Column names Intensity, Transmission
    ...
    dummy <- list(dummy=NULL)
   	data <- c(data, tmp, dummy)  #add Intensity, Transmission and a NULL dummy column for future use
   	NC <- length(data)
...
## magnification_transfer_lens
## signal mode == 'pulse counting'
## signal_collection_time == in seconds; in comments time could be in ms. (eg:Dwell Time(ms): 100 == 0.1)
...
## acq_time <- as.numeric(signal_collection_time) * as.numeric(num_scans_block) * as.numeric(num_ordinate_values)/as.numeric(corresponding_variables_num)
    ...
    if(length(data)==3){ #Analyzer transmission function is present
       data[[2]] <- data[[2]]/data[[3]]  #correction for the Analyzer transmission function
    }
