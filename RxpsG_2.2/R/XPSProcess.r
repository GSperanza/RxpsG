...
#  addSpring(Pgroup1)
  ...
  gbutton("CLOSE", container = Pgroup1, handler=function(h,...){dispose(Pgroup1)}  )


#----- plot area -----
  GraphGroup <- ggroup(container = MainGroup)
  GraphFrame <- gframe("Interactive Graphical Window", container=GraphGroup)
  ...
  img <- tkrplot(getToolkitWidget(GraphFrame), fun = draw.plot, hscale=HS, vscale=VS)
  add(GraphGroup, img)
  ## interactivity
