...
#      indx <- which(YYY == Min,  arr.ind = TRUE)
#      if (length(indx) > 1) indx <- indx[as.integer(length(indx)/2)] # if index is a vector select the index value corresponding to LL/2
#      x1 <- indx
#      Max <- max(YYY)      #compute averaged noise free value around the data maximum
#      indx <- which(YYY == Max, arr.ind = TRUE)
#      if (length(indx) > 1) indx <- indx[as.integer(length(indx)/2)] # if index is a vector select the index value corresponding to LL/2
#      x2 <- indx
...
#      Min <- min(data)
#      Max <- max(data)
#      data <- (data-Min)/(Max-Min)   #filtered normalized data are not in the range [0 : 1]
      data <- normalize(data)
      Min <- AmpliRng[1]
      Max <- AmpliRng[2]
