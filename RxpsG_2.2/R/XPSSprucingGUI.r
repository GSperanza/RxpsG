...
#Coreline boundaries
  ...
  SpectList <- XPSSpectList(activeFName)  #activeFName is seen in .GlobalEnv
...
#====== OPTIONS SECTION =======
  ...
  gframe10 <- gframe(text="SELECT CORELINE", spacing=5, container=OptGroup)
...
#  font(HelpLab)<-list(family="sans",size=12)
  ...
  gframe22 <- gframe(text = " SELECT DATA ", horizontal = FALSE, container = OptGroup)
  ...
  gframe23 <- gframe(text = " Sprucing ", container = OptGroup)
...
#====== PLOT SECTION =======
  SPlayout[1,2] <- SPFrame <- gframe("Interactive Graphical Window", container=SPlayout)
  SPlayout[1,2] <- img <- tkrplot(getToolkitWidget(SPFrame), fun = draw.plot, hscale=plot_win, vscale=plot_win)
  addSpring(MainGroup)
